...
let tipoMovimentacaoAtual = 'Entrada';
  ...
  function selecionarModoRegistro(modo) {
    ...
    document.getElementById('labelTipoMovimentacao').textContent = isSOS ? 'MovimentaÃ§Ã£o de SOS' : 'Tipo de movimentaÃ§Ã£o';
    document.getElementById('textoBtnEntrada').textContent = isSOS ? 'Retorno' : 'Entrada';
    document.getElementById('textoBtnSaida').textContent = 'SaÃ­da';
  ...
  function renderizarSelecaoViaturasSOS() {
    ...
    const situacaoNecessaria = retorno ? 'Em ocorrÃªncia' : 'No quartel';
    ...
    document.getElementById('tituloSelecaoSOS').textContent = retorno
      ...
      : 'Selecionar viaturas para saÃ­da';
    ...
    if (!disponiveis.length) {
      lista.appendChild(criarEstadoVazioPainel(
        retorno
          ? 'NÃ£o hÃ¡ viaturas em ocorrÃªncia.'
          : 'NÃ£o hÃ¡ viaturas disponÃ­veis no quartel.'
      ));
  ...
  function alternarViaturaSOS(viatura, selecionada) {
    if (selecionada) {
      ...
      selecoesViaturasSOS[viatura.ID_Viatura] = {
        ...
        ID_Condutor: tipoMov…22326 tokens truncated…dadosCodigoComandante = { email: email, militar: null, ticketAssuncao: '' };
}
  ...
  google.script.run
    .withSuccessHandler((resposta) => {
      ...
      if (ciclo && resposta && resposta.cicloInicio && resposta.cicloFim) {
        ciclo.textContent = 'Ciclo: ' + resposta.cicloInicio + ' atÃ© ' + resposta.cicloFim + '. Exibindo os 30 registros mais recentes.';
      }
    ...
    .withFailureHandler((erro) => {
      if (!silencioso) mostrarMensagem('Erro ao carregar movimentaÃ§Ãµes: ' + erro.message, 'erro');
      if (botao) {
        botao.disabled = false;
        botao.textContent = 'Atualizar';
      }
...
function atualizarTelaToqueFogo() {
  ...
  periodoEl.textContent = (periodo.nome || 'PerÃ­odo atual') + ' â€¢ ' + (periodo.faixa || '');
  ...
  if (toque) {
    ...
    statusEl.innerHTML = 'Toque de Fogo atual:<br>' + escaparHtml(toque.Nome_Toque) +
      ' â€” RG ' + escaparHtml(toque.RG_Toque);
    ...
    statusEl.textContent = 'Nenhum Toque de Fogo assumiu o perÃ­odo ' + (periodo.faixa || 'atual') + '.';
  ...
  if (cobertura) {
    ...
    coberturaEl.innerHTML = '<strong>Cobertura ativa</strong><br>' +
      escaparHtml(cobertura.Nome_Toque) + ' estÃ¡ no posto por ' +
      escaparHtml(cobertura.Nome_Guarda_Titular) + '.<br>' +
      'InÃ­cio: ' + escaparHtml(cobertura.DataHora_Inicio || '-');
  } else {
...
function mostrarAreaTrocaToqueFogo() {
  ...
  mostrarMensagem('Valide o e-mail do militar que assumirÃ¡ o Toque de Fogo neste perÃ­odo.', 'sucesso');
}
...
function enviarCodigoToqueFogo() {
  ...
  google.script.run
    .withSuccessHandler((resposta) => {
      ...
      mostrarMensagem('CÃ³digo enviado para o e-mail do Toque de Fogo.', 'sucesso');
    })
    .withFailureHandler((erro) => mostrarMensagem('Erro ao enviar cÃ³digo: ' + erro.message, 'erro'))
...
function validarCodigoToqueFogo() {
  ...
  if (!email || !codigo) {
    mostrarMensagem('Informe o e-mail e o cÃ³digo.', 'erro');
    return;
  }
  google.script.run
    .withSuccessHandler((resposta) => {
      ...
      if (resposta.encontradoNoEfetivo && resposta.militar) {
        area.innerHTML = 'Militar identificado:<br>' + escaparHtml(resposta.militar.Nome) +
          ' â€” RG ' + escaparHtml(resposta.militar.RG);
        manual.classList.add('oculto');
      } else {
        area.textContent = 'E-mail validado. Informe RG e nome do militar.';
        manual.classList.remove('oculto');
      }
      mostrarMensagem('CÃ³digo validado com sucesso.', 'sucesso');
    })
    .withFailureHandler((erro) => mostrarMensagem('Erro ao validar cÃ³digo: ' + erro.message, 'erro'))
    ...
    .withSuccessHandler((resposta) => {
      ...
      if (resposta.requerConfirmacaoTroca && resposta.toqueAtivo) {
        abrirModalConfirmacao(
          'Toque de Fogo jÃ¡ assumido',
          'O perÃ­odo estÃ¡ assumido por <strong>' + escaparHtml(resposta.toqueAtivo.Nome_Toque) +
...
function confirmarRetomadaPosto() {
  abrirModalConfirmacao(
    'Retomar posto',
    'Confirma que o militar da hora retornou e estÃ¡ retomando o posto da Guarda?',
    () => retomarPostoAposSOS()
  );
}
...
function confirmarAssuncaoHoraToque() {
  abrirModalConfirmacao(
    'Assumir hora',
    'Confirma que o Toque de Fogo estÃ¡ assumindo imediatamente o posto do militar da hora?',
    () => assumirHoraToqueFogo()
  );
}
